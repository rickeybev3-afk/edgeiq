...
4.  **Nightly Tiered P&L Refresh:** `nightly_tiered_pnl_refresh.py` runs as a persistent Replit workflow ("Nightly Tiered P&L Refresh"). On startup it immediately calls `run_tiered_pnl_backfill.py --backtest-only` to fill any rows with `tiered_pnl_r = NULL`, then sleeps until 00:05 ET each night and repeats. This ensures `backtest_sim_runs.tiered_pnl_r` stays current even if batch backtest runs introduce new NULL rows. Primary computation is still done inline at insert time by `batch_backtest.py`; the nightly job is the automated safety net.
5.  **Alpaca Execution Layer:** Integrated for automated bracket order placement (entry, stop loss, take profit) based on IB levels. Supports both paper and live trading via configuration.
6.  **P&L Simulation:** Three simulation scenarios (`pnl_r_sim` for MFE, `eod_pnl_r` for EOD hold, `tiered_pnl_r` for 50/25/25 ladder) are implemented in `backend.py` to evaluate strategy performance.
7.  **UI/UX:** Streamlit is used for the front-end, featuring dark mode and Plotly charts. Interactive filter simulation (`/filter_sim`) allows users to adjust parameters and view historical performance.
8.  **Data Column Semantics:** Strict definitions for data columns like `predicted`, `actual_outcome`, `win_loss`, `follow_thru_pct`, `pnl_r_sim`, `eod_pnl_r`, `tiered_pnl_r`, `close_price`, `ib_range_pct`, and `vwap_at_ib` are enforced to ensure data integrity and correct interpretation.
