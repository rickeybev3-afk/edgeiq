...
--------------------------
Analyses settled squeeze screener-pass paper trades (screener_pass='squeeze') and
recommends a calibrated position-size multiplier for _SP_MULT_TABLE in
paper_trader_bot.py.

Run once at least 30 squeeze trades have settled in paper_trades.

Methodology (mirrors the 5-year backtest used for other passes):
  1. Pull all settled squeeze rows (tiered_pnl_r IS NOT NULL).
...
def _recommend_mult(squeeze_exp: float, gap_exp: float) -> float:
    """
    ...
    Edge-case handling (returns conservative fixed values, never crashes):
      ...
      squeeze_exp <= 0 → strategy has negative/zero expectancy; return 0.70
...
def _self_test() -> None:
    ...
    cases = [
        ...
        ("zero squeeze",               0.000, 0.327, 0.70),
        ("negative squeeze",          -0.100, 0.327, 0.70),
        ("zero gap anchor",            0.400, 0.000, 1.00),
        ("negative gap anchor",        0.400,-0.100, 1.00),
    ]
    all_ok = True
    for label, sq_exp, ga_exp, expected in cases:
        result = _recommend_mult(sq_exp, ga_exp)
        ok = abs(result - expected) < 0.001
        print(f"  {'OK  ' if ok else 'FAIL'} {label}: _recommend_mult({sq_exp}, {ga_exp}) = {result} (expected {expected})")
        if not ok:
            all_ok = False
    if all_ok:
...
def main() -> None:
    ...
    if sq["n"] < MIN_TRADES:
        print(
            f"\n⛔  Only {sq['n']} settled squeeze trades found — "
            f"minimum is {MIN_TRADES}.\n"
            f"    Re-run this script once {MIN_TRADES - sq['n']} more trades settle.\n"
            f"    _SP_MULT_TABLE['squeeze'] remains at 1.00× (safe baseline)."
        )
        sys.exit(0)
    ...
    print(f"\nStatistics:")
    for label, s in [("squeeze", sq), ("gap", ga), ("other", ot), ("trend", tr)]:
    ...
    sq_exp = sq["expectancy"]

    if ga_exp is None:
        print("\nWARNING: no settled 'gap' trades — cannot anchor multiplier. Using 1.00×.")
        rec_mult = 1.00
    else:
        rec_mult = _recommend_mult(sq_exp, ga_exp)

    date_range = ""
    if squeeze_rows:
        dates = sorted(r.get("trade_date", "") for r in squeeze_rows if r.get("trade_date"))
        if dates:
            date_range = f"{dates[0]} → {dates[-1]}"

    print(f"\n{'='*60}")
    print(f"RECOMMENDATION:  _SP_MULT_TABLE['squeeze'] = {rec_mult:.2f}")
    print(f"{'='*60}")
    wr_str = f"{sq['wr']:.1%}" if sq["wr"] is not None else "N/A"
    print(
        f"\nData citation to paste above _SP_MULT_TABLE in paper_trader_bot.py:\n"
        f"\n"
        f"#   'squeeze' ({date_range}): "
        f"{sq['n']} trades, {wr_str} WR / {_fmt(sq['avg_r'])}R avg → "
        f"{rec_mult:.2f}×"
    )
    if ga_exp is not None and ga_exp > 0 and sq_exp is not None and sq_exp > 0:
        raw = sq_exp / ga_exp
        print(
            f"\nMethodology: sqrt-dampened ratio vs 'gap' expectancy "
            f"({sq_exp:+.3f}R ÷ {ga_exp:+.3f}R = "
            f"{raw:.3f} → sqrt → {math.sqrt(raw):.3f} → "
            f"clamped [0.70, 1.30] → rounded to nearest 0.05 → {rec_mult:.2f}×)"
        )
    else:
        print(
            f"\nMethodology: edge-case path (squeeze_exp={sq_exp}, gap_exp={ga_exp}) — "
            f"conservative fixed value {rec_mult:.2f}× returned."
        )
    print(
        f"\nNext step: update paper_trader_bot.py:\n"
        f"    \"squeeze\": {rec_mult:.2f},   "
        f"# {sq['n']} trades {date_range}, "
        f"{wr_str} WR / {_fmt(sq['avg_r'])}R"
    )
