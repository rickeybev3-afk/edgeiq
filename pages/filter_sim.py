...
with _r1c4:
    ...
    if bot_mode:
        _bot_risk_display = min(float(pnl_equity) * 0.021, 4000.0)
        st.metric("Risk per trade", f"${_bot_risk_display:,.0f}",
                  help="Bot Mode: 2.1% of account size, capped at $4,000. Scales as equity grows.")
    else:
